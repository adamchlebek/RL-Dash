...
export const GameDetailsModal = ({ gameId, isOpen, onClose }: GameDetailsModalProps) => {
    ...
    const StatsTable = ({ players }: StatsTableProps) => {
        ...
        const highestScore = Math.max(
            ...(gameDetails?.teams.blue.players
                .concat(gameDetails?.teams.orange.players)
                .map((p) => p.score) || [0])
        );
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-border border-b">
                            <th className="text-foreground py-2 text-left">Player</th>
                            <th className="text-foreground py-2 text-right">Score</th>
                            <th className="text-foreground py-2 text-right">Goals</th>
                            <th className="text-foreground py-2 text-right">Assists</th>
                            <th className="text-foreground py-2 text-right">Saves</th>
                            <th className="text-foreground py-2 text-right">Shots</th>
                            <th className="text-foreground py-2 text-right">Demos</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <div className="flex items-center justify-center gap-12 rounded-lg bg-zinc-800/50 p-8">
                                <div className="text-center">
                                    ...
                                    <div className="text-4xl font-bold">
                                        {gameDetails.teams.blue.goals}
                                    </div>
                                </div>
                                <div className="text-center">
                                    <div className="space-y-0.5">
                                        <div className="text-sm text-zinc-400">
                                            {formatDate(gameDetails.date).date}
                                        </div>
                                        <div className="text-sm text-zinc-400">
                                            {formatDate(gameDetails.date).time}
                                        </div>
                                    </div>
                                </div>
                                <div className="text-center">
