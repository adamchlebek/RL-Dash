...
const calculateShootingPercentage = (
  goals: number | null,
  shots: number | null,
): number => {
    ...
    return {
      ...
      players: team.players.map(
        (player: RawPlayer): PlayerDetails => ({
          ...
          shootingPercentage: calculateShootingPercentage(
            player.goals,
            player.shots,
          ),
          ...
          stats: {
            ...
            movement: {
              avgSpeed: 0,
              totalDistance: convertUnitsToMiles(
                player.movementTotalDistance || 0,
              ),
      ...
      stats: {
        ...
        movement: {
          ...
          timeInAir: calculateAirTime(
            team.movementTimeLowAir,
            team.movementTimeHighAir,
          ),
        },
        positioning: {
