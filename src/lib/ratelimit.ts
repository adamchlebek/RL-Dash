...
class BallchasingRateLimiter {
    ...
    if (DEBUG_MODE) {
      setInterval(() => {
        if (this.totalProcessed > 0) {
          ...
          console.log(`[Ballchasing Rate Limiter] Stats: ${this.totalProcessed} requests processed, ${this.queue.length} in queue, avg wait: ${avgWaitTime.toFixed(2)}ms`);
          // Reset counters
          this.totalProcessed = 0;
          this.totalWaitTime = 0;
        }
  ...
  private refillTokens(): void {
    ...
    const hoursSinceLastRefill = (now - this.hourlyLastRefill) / (1000 * 60 * 60);
    const hourlyTokensToAdd = hoursSinceLastRefill * MAX_REQUESTS_PER_HOUR;
    if (hourlyTokensToAdd >= 1) {
      this.hourlyTokens = Math.min(MAX_REQUESTS_PER_HOUR, this.hourlyTokens + hourlyTokensToAdd);
      this.hourlyLastRefill = now;
    }
    
    if (DEBUG_MODE && this.queue.length > 0) {
      console.log(`[Ballchasing Rate Limiter] Tokens: ${this.tokens.toFixed(2)}/sec, ${this.hourlyTokens.toFixed(2)}/hour, Queue: ${this.queue.length}`);
    }
  }
  ...
  private async processQueue(): Promise<void> {
    ...
    while (this.queue.length > 0) {
      if (!this.hasTokens()) {
        ...
        if (DEBUG_MODE) {
          console.log(`[Ballchasing Rate Limiter] Waiting ${secDelay.toFixed(0)}ms for next token`);
        }
      ...
      if (DEBUG_MODE && waitTime > 500) {
        console.log(`[Ballchasing Rate Limiter] Request for ${request.ballchasingId} waited ${waitTime}ms in queue`);
      }
      ...
      try {
        ...
        if (axios.isAxiosError(error) && error.response?.status === 429) {
          ...
          console.warn(`[Ballchasing Rate Limiter] Rate limit exceeded, re-queuing request for ${request.ballchasingId}`);
  ...
  public async executeWithRateLimit<T>(
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      this.processQueue().catch(err => {
        console.error('[Ballchasing Rate Limiter] Error processing rate limit queue:', err);
      });
    });
  }
