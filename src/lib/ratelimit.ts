...
export function consumeToken(key: string, tokenLimit: number, refillRate: number): boolean {
    ...
    if (tokensToAdd > 0) {
        limiter.tokensRemaining = Math.min(tokenLimit, limiter.tokensRemaining + tokensToAdd);
