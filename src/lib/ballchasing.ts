...
export const uploadReplay = async (file: Buffer, fileName: string): Promise<ReplayResponse> => {
  ...
  try {
    if (DEBUG_MODE) console.log(`[Ballchasing API] Queuing upload for ${fileName}...`);
    ...
    const response = await ballchasingRateLimiter.executeWithRateLimit(
      ...
      async () => {
        if (DEBUG_MODE) console.log(`[Ballchasing API] Executing upload for ${fileName}...`);
    ...
    if (axios.isAxiosError(error) && error.response?.status === 409) {
      ...
      console.log('Duplicate replay detected, using existing replay ID:', error.response.data.id);
      return {
    ...
    console.error('Error uploading to Ballchasing:', error instanceof Error ? error.message : 'Unknown error');
    throw error;
  }
};
 ...
 */
export const checkReplayStatus = async (replayId: string): Promise<BallchasingStatus> => {
  try {
    if (DEBUG_MODE) console.log(`[Ballchasing API] Queuing status check for replay: ${replayId}`);
    
    // Use rate limiter for the status check request
    const response = await ballchasingRateLimiter.executeWithRateLimit(
      replayId,
      async () => {
        if (DEBUG_MODE) console.log(`[Ballchasing API] Executing status check for replay: ${replayId}`);
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error(`Replay ${replayId} check failed:`, error instanceof Error ? error.message : 'Unknown error');
    }
    return 'failed';
  }
};
 ...
 */
export const fetchFullReplayData = async (ballchasingId: string): Promise<BallchasingReplayResponse> => {
  try {
    if (!API_KEY) {
      throw new Error('Ballchasing API key not found');
    }

    if (DEBUG_MODE) console.log(`[Ballchasing API] Queuing full data fetch for replay: ${ballchasingId}`);
    
    // Use rate limiter for fetching full replay data
    const response = await ballchasingRateLimiter.executeWithRateLimit(
      ballchasingId,
      async () => {
        if (DEBUG_MODE) console.log(`[Ballchasing API] Executing full data fetch for replay: ${ballchasingId}`);
    ...
    console.error(`Error fetching full replay data for ${ballchasingId}:`, error);
    throw error;
  }
}; 