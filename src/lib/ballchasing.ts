...
export const checkReplayStatus = async (replayId: string): Promise<BallchasingStatus> => {
    try {
        if (DEBUG_MODE)
            console.log(`[Ballchasing API] Queuing status check for replay: ${replayId}`);

        // Use rate limiter for the status check request
        const response = await ballchasingRateLimiter.executeWithRateLimit(replayId, async () => {
            if (DEBUG_MODE)
                console.log(`[Ballchasing API] Executing status check for replay: ${replayId}`);
...
export const fetchFullReplayData = async (
    ...
    try {
        ...
        if (DEBUG_MODE)
            console.log(`[Ballchasing API] Queuing full data fetch for replay: ${ballchasingId}`);
