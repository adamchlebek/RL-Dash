    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      <div className="bg-zinc-800/50 backdrop-blur-sm rounded-xl p-6 border border-zinc-700/50">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold">{new Date(game.date).toLocaleDateString()}</h1>
        ...
        <div className="mt-6 flex justify-center items-center gap-4 text-3xl font-bold">
          <div className="flex gap-2">
            {game.winningTeam.map(player => (
              <span key={player} className="bg-green-900/50 px-3 py-1 rounded-lg">{player}</span>
            ))}
          </div>
          <span className="text-4xl font-mono">{game.score}</span>
          <div className="flex gap-2">
            {game.losingTeam.map(player => (
              <span key={player} className="bg-red-900/50 px-3 py-1 rounded-lg">{player}</span>
            ))}
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {allPlayers.map(player => (
                <tr key={player} className="border-b border-zinc-700 last:border-0">
